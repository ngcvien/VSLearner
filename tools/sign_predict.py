...
dot = mp_drawing.DrawingSpec(color=(255,255,255), thickness=1, circle_radius=4)
line = mp_drawing.DrawingSpec(color=(255,255,255), thickness=1, circle_radius=0)
...
with mp_hol.Holistic(static_image_mode=False,
    ...
    while True:
        ...
        # ----- draw shoulders + arms (both sides) using pose landmarks -----
            if res.pose_landmarks:
                mp_drawing.draw_landmarks(frame, res.pose_landmarks, mp_hol.POSE_CONNECTIONS,
                              landmark_drawing_spec=dot,
                              connection_drawing_spec=line)
            if res.left_hand_landmarks:
                mp_drawing.draw_landmarks(frame, res.left_hand_landmarks, mp.solutions.hands.HAND_CONNECTIONS,
                                landmark_drawing_spec=dot,
                                connection_drawing_spec=line)
            if res.right_hand_landmarks:
                mp_drawing.draw_landmarks(frame, res.right_hand_landmarks, mp.solutions.hands.HAND_CONNECTIONS,
                                landmark_drawing_spec=dot,
                                connection_drawing_spec=line)
